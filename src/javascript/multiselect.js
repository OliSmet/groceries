$(document).ready(function () {
  ...
  for (i = 0; i < l; i++) {
    ...
    for (j = 1; j < ll; j++) {
      ...
      c.innerHTML = selElmnt.options[j].innerHTML.replace(/%%(.*?)%%/g, '<span class="phoneColor $1">$1</span>');
      c.addEventListener("click", function (e) {
        ...
        for (i = 0; i < sl; i++) {
          if (s.options[i].innerHTML.replace(/%%(.*?)%%/g, '<span class="phoneColor $1">$1</span>') == this.innerHTML) {
            s.selectedIndex = i;
